...
// 固定的六色调色板
const rgbPalette = [
  // Use the panel's measured ink colors for distance calculations and preview.
  { name: "黄色", r: 245, g: 205, b: 30, value: 0x02 },
  { name: "绿色", r: 35, g: 150, b: 55, value: 0x06 },
  { name: "蓝色", r: 25, g: 105, b: 190, value: 0x05 },
  { name: "红色", r: 210, g: 35, b: 35, value: 0x03 },
...
function ditherImage(imageData, alg, strength, mode) {
  // Six-color panels have a wider effective quantization step; reduce error
  // diffusion slightly to avoid harsh speckling in the color regions.
  ...
  if (mode === 'sixColor') {
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const color = rgbPalette.find(c => c.value === colorValue) || rgbPalette.find(c => c.value === 0x01); // 默认白色
